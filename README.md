...
- v2.7.1 新增国内四线路丢包率监控与历史图表，已预留 GPU 字段与图表展示；升级后请在后台点击 **升级数据库** 并重新安装/升级探针，否则新图表可能提示数据库结构未升级
...
- 📊 **实时监控**：CPU、内存、磁盘、网络、进程数、连接数、负载均衡（已预留 GPU 字段与展示）
- 📈 **历史图表**：支持7天历史数据查看
- 🌍 **全球地图**：可视化展示服务器分布
...
- 实时 CPU/内存/磁盘/网络/负载（GPU 字段与图表已预留，待探针上报实现后显示）
- 7天历史趋势图
- 鼠标悬停查看具体时间点的数值
- 国内四线路延迟与丢包率追踪
